...
describe('MerkleDistributor', () => {
  ...
  describe('#token', () => {
    it('returns the token address', async () => {
      const distributor = await deployContract(wallet0, Distributor, [token.address, ZERO_BYTES32, wallet0.address, 30], overrides)
  ...
  describe('#claim', () => {
    ...
    describe('two account tree', () => {
      ...
      beforeEach('deploy', async () => {
        ...
        distributor = await deployContract(wallet0, Distributor, [token.address, tree.getHexRoot(), wallet0.address, 30], overrides)
      ...
      it('cannot claim if the airdrop time is over', async () => {
        function timeout(ms:number) {
            return new Promise(resolve => setTimeout(resolve, ms));
        }
        const distributor = await deployContract(wallet0, Distributor, [token.address, tree.getHexRoot(), wallet0.address, 1], overrides)
        await token.setBalance(distributor.address, 201);
        ...
        await expect(distributor.claim(
          ...
          'TokenDistributor: airdrop duration was passed.'
        )
      })
  ...
  describe('parseBalanceMap', () => {
    ...
    beforeEach('deploy', async () => {
      ...
      claims = innerClaims
      distributor = await deployContract(wallet0, Distributor, [token.address, merkleRoot, wallet0.address, 30], overrides)
