...
contract TokenVesting is Initializable, Ownable, BlackholePrevention {
    ...
    struct VestingInfo {
        ...
        uint256 vestingPeriod; // duration in seconds
    ...
    function lock(
        ...
        uint256 _vestingPeriod // in seconds
    ) external onlyOwner {
        VestingInfo[] storage vestingArr = vestings[_addr];
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > vestingArr[_id].vestingPeriod + vestingArr[_id].unlockedFrom) return 0;
        
        uint256 gap = block.timestamp.sub(vestingArr[_id].unlockedFrom);
        uint256 releasable = vestingArr[_id].totalAmount.mul(gap).div(
