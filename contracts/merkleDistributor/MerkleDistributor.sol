...
contract MerkleDistributor is IMerkleDistributor {
    ...
    address public immutable timeLockContract;
    
    uint public contractActiveDuration = 180 days;
    ...
    constructor(address token_, bytes32 merkleRoot_, address _timeLock, uint duration) public {
        token = token_;
        merkleRoot = merkleRoot_;
        timeLockContract = _timeLock;
        deployTime = block.timestamp;
        if (duration > 0) {
            contractActiveDuration = duration;
        }
    }
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
        require(block.timestamp <= deployTime + contractActiveDuration, 'TokenDistributor: airdrop duration was passed.');
    ...
    **/
    function transferUnclaimed() public {
        require(block.timestamp >= deployTime + contractActiveDuration, 'TokenDistributor: Contract is still active.');
